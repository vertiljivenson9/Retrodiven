...
const App = () => {
  const [os, setOs] = useState({ 
    isOff: false, isLocked: true, wallpaper: DEFAULT_WALLPAPER, 
    authorizedUser: 'Vertil', windows: [], files: [], 
    processes: [{pid:101, name:'Kernel', status:'Running', memoryUsage:128}] 
  });
    ...
    <div className="fixed inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${os.wallpaper})` }}>
      ...
      {os.windows.map(win => (
        <Window key={win.id} window={win} isActive={os.activeWindowId === win.id} onClose={id => setOs(p => ({...p, windows: p.windows.filter(w=>w.id!==id)}))} onFocus={id => setOs(p => ({...p, activeWindowId: id}))}>
          ...
          {win.appId === 'store' && <Store onInstallApp={a => setOs(p => ({...p, installedApps: [...(p.installedApps||[]), a]}))} />}
          {win.appId === 'reggedit' && <VertilReggedit processes={os.processes} services={[]} />}
        </Window>
      ))}
    </div>
  );
